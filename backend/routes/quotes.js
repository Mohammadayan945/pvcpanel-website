...
router.get('/', auth, async (req, res) => {
  ...
  try {
    let q = 'SELECT q.*, u.name as assigned_name FROM quote_requests q LEFT JOIN users u ON q.assigned_to = u.id';
    let countQ = 'SELECT COUNT(*) as total FROM quote_requests';
    const params = [];
    if (status) { q += ' WHERE q.status = ?'; countQ += ' WHERE status = ?'; params.push(status); }
...
router.patch('/:id', auth, async (req, res) => {
  const { status, notes, quote_amount, site_visit_date, assigned_to } = req.body;
  try {
    await pool.query(
      'UPDATE quote_requests SET status=COALESCE(?,status), notes=COALESCE(?,notes), quote_amount=COALESCE(?,quote_amount), site_visit_date=COALESCE(?,site_visit_date), assigned_to=COALESCE(?,assigned_to) WHERE id=?',
      [status||null, notes||null, quote_amount||null, site_visit_date||null, assigned_to||null, req.params.id]
    );
